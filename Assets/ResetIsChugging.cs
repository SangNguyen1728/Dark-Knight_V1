...
public class ResetIsChugging : StateMachineBehaviour
{
    ...
    override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        // If i am out of flask play empty animation(DO IT Later)
        if(player.playerNetworkManager.isChugging.Value && player.IsOwner)
        {
            ...
            if (currentFlask.healthFlask)
            {
                ...
                {
                    player.playerAnimatorManager.PlayTargetActionAnimtion(currentFlask.emptyFlaskAnimation, false, false, true, true, false);
            ...
            // If I am out of flask, instantiate the empty flask
            if (currentFlask.healthFlask)
            {
                if(player.playerNetworkManager.remainingHealthFlasks.Value <= 0)
                {
            ...
            {
                // For Future: for remain Forcus Point
